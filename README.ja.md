...
2. **⚡ 実機 TypeSafe Jev 連携（Vercel 無料枠で利用可能）**:
   - Vercel AI Gateway の無料枠（Free / Hobby プラン）で提供されている本物の `typesafe-ai/jev` モデルに直結。高額な GPU サーバーや有料サブスクリプションを契約することなく、誰でも即座に最先端の型安全 AI 物理制御を実験・ベンチマークできます。非同期意思決定パイプラインにより、HTTP 通信遅延中も 60 FPS 物理ループをスムーズに維持します。
3. **🎥 常時 MP4 動画自動録画**:
